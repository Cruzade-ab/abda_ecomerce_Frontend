    ...
    (async () => {
      try {
        const response = await fetch(
          "http://localhost:4000/api/userOperations/getUser",
          {
            credentials: "include",
          }
        );
        if (response.ok) {
          ...
          setMessage(
            `${content.name} ${content.last_name} is logged in with an email: ${content.email}. Its roles is the # ${content.role_id}`
          );
          setIsLoggedIn(true);
          console.log(content);
        } else {
